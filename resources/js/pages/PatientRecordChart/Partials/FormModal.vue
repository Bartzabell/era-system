        ...
        <form @submit.prevent="submitForm" class="p-4 bg-form-body dark:bg-gray-800 max-h-[85vh] space-y-6">
            ...
            <section>
                ...
                <div class="p-3 border border-dashed border-gray-400 rounded-md">
                    ...
                    />
                    <p v-if="form.errors.incident_report_id" class="text-sm text-red-500 mt-1">{{ form.errors.incident_report_id }}</p>
                    ...
                    <div class="lg:col-span-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button
                                ...
                                :key="opt.value"
                                type="button"
                                @click="form.case_type = form.case_type === opt.value ? null : opt.value as any"
                                :class="[
                        ...
                        <p v-if="form.errors.case_type" class="text-sm text-red-500 mt-1">{{ form.errors.case_type }}</p>
                    </div>
                </div>
            </section>
            ...
            <section>
                ...
                <div class="grid grid-cols-2 gap-3 p-3 border border-dashed border-gray-400 rounded-md lg:grid-cols-4">
                    <div><CustomInput name="Time Dispatch"              type="time" v-model="form.time_dispatch" /></div>
                    <div><CustomInput name="Arrived on Scene"           type="time" v-model="form.time_arrived_on_scene" /></div>
                    <div><CustomInput name="En-route to Hospital"       type="time" v-model="form.time_enroute_to_hospital" /></div>
                    <div><CustomInput name="Arrival in Hospital"        type="time" v-model="form.time_arrival_in_hospital" /></div>
                    <div><CustomInput name="Departure from Hospital"    type="time" v-model="form.time_departure_in_hospital" /></div>
                    <div><CustomInput name="Back to Base"               type="time" v-model="form.time_back_to_base" /></div>
                    <div><CustomInput name="Mileage Before Run (km)"    type="number" step="0.01" v-model="form.mileage_before_run" /></div>
                    <div><CustomInput name="Mileage Back to Base (km)"  type="number" step="0.01" v-model="form.mileage_back_to_base" /></div>
                </div>
            </section>
            ...
            <section>
                ...
                <div class="grid grid-cols-1 gap-3 p-3 border border-dashed border-gray-400 rounded-md lg:grid-cols-3">
                    <div><CustomInput name="Dispatcher"        v-model="form.dispatcher" /></div>
                    <div><CustomInput name="Unit"              v-model="form.unit" /></div>
                    <div><CustomInput name="Transport Officer" v-model="form.transport_officer" /></div>
                    <div><CustomInput name="Team Leader"       v-model="form.team_leader" /></div>
                    <div class="lg:col-span-2"><CustomInput name="Medics" v-model="form.medics" /></div>
                </div>
            </section>
            ...
            <section>
                ...
                <div class="grid grid-cols-1 gap-3 p-3 border border-dashed border-gray-400 rounded-md lg:grid-cols-3">
                    ...
                    <div><CustomInput name="Last Name"   v-model="form.last_name"   required /></div>
                    <div><CustomInput name="First Name"  v-model="form.first_name"  required /></div>
                    <div><CustomInput name="Middle Name" v-model="form.middle_name" /></div>
                    ...
                    <div>
                        ...
                        <select v-model="form.gender" class="w-full px-3 py-2 border border-gray-300 rounded-md text-base dark:bg-gray-700 dark:text-white">
                    ...
                    <div>
                        ...
                        <select v-model="form.civil_status" class="w-full px-3 py-2 border border-gray-300 rounded-md text-base dark:bg-gray-700 dark:text-white">
                            <option :value="null">— Select —</option>
                            <option v-for="opt in civilStatusOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
                        </select>
                    </div>
                    ...
                    <div><CustomInput name="Date of Birth"   type="date"   v-model="form.date_of_birth" /></div>
                    <div><CustomInput name="Contact #"                     v-model="form.contact_number" /></div>
                    <div><CustomInput name="Religion"                      v-model="form.religion" /></div>
                    <div><CustomInput name="Insurance / HMO Provider"      v-model="form.insurance_hmo_provider" /></div>
                    <div><CustomInput name="Insurance / HMO Number"        v-model="form.insurance_hmo_number" /></div>
            ...
            <section>
                ...
                <div class="p-3 border border-dashed border-gray-400 rounded-md space-y-4">
                    ...
                    <!-- Mental Status -->
                    <div>
                        <label class="block m-1 text-base font-medium text-gray-600 dark:text-gray-200">Mental Status</label>
                        <div class="flex flex-wrap gap-2">
                            <label v-for="opt in mentalStatusOptions" :key="opt.value"
                                class="flex items-center gap-1 text-sm text-gray-600 dark:text-gray-300 cursor-pointer">
                                <input type="checkbox" :value="opt.value" v-model="form.mental_status" class="rounded" />
                                {{ opt.label }}
                            </label>
                        </div>
                    </div>
                    ...
                    <!-- Stroke Signs -->
                    <div>
                        <label class="block m-1 text-base font-medium text-gray-600 dark:text-gray-200">Stroke Signs</label>
                    ...
                    <!-- Interventions -->
                    <div>
                        <label class="block m-1 text-base font-medium text-gray-600 dark:text-gray-200">Interventions</label>
                        <div class="grid grid-cols-2 gap-1 lg:grid-cols-4">
                            <label v-for="opt in interventionOptions" :key="opt.value"
                                class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 cursor-pointer">
                                <input type="checkbox" :value="opt.value" v-model="form.interventions" class="rounded" />
                                {{ opt.label }}
                            </label>
                        </div>
                    ...
                    <!-- Transport Priority -->
                    <div>
                        <label class="block m-1 text-base font-medium text-gray-600 dark:text-gray-200">Transport Priority</label>
                        <div class="flex flex-wrap gap-2">
                            <button
                                v-for="opt in transportPriorityOptions"
                                :key="opt.value"
                                type="button"
                        ...
                        <p v-if="form.errors.transport_priority" class="text-sm text-red-500 mt-1">{{ form.errors.transport_priority }}</p>
                    </div>
            ...
            <section>
                ...
                <div class="grid grid-cols-1 gap-3 p-3 border border-dashed border-gray-400 rounded-md lg:grid-cols-2">
                    ...
                    <div>
                        <label class="block m-1 text-base text-gray-600 dark:text-gray-200">S — Signs &amp; Symptoms</label>
                    ...
                    <div>
                        <label class="block m-1 text-base text-gray-600 dark:text-gray-200">P — Pertinent Past History</label>
                    ...
                    <div>
                        <label class="block m-1 text-base text-gray-600 dark:text-gray-200">E — Events Leading to Illness / Injury</label>
                    ...
                    <!-- Trauma sub-type -->
                    <div>
                        <label class="block m-1 text-base font-medium text-gray-600 dark:text-gray-200">Trauma Case Type</label>
                        <div class="flex flex-col gap-1">
                            <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 cursor-pointer">
                                <input type="checkbox" value="vehicular_accident"    v-model="form.trauma_type" class="rounded" /> Vehicular Accident
                            </label>
                            <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 cursor-pointer">
                                <input type="checkbox" value="trauma_of_other_cause" v-model="form.trauma_type" class="rounded" /> Trauma of Other Cause
                            </label>
                        </div>
                    </div>
                    ...
                    <!-- DCAPBTLS -->
                    <div>
                        <label class="block m-1 text-base font-medium text-gray-600 dark:text-gray-200">DCAPBTLS Findings</label>
                ...
                <div class="p-3 border border-dashed border-gray-400 rounded-md space-y-3">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="text-sm font-medium text-gray-600 dark:text-gray-300">Vital Signs Log (time-series)</label>
                            <button type="button" @click="addVitalRow"
                                class="px-2 py-1 text-xs bg-blue-500 hover:bg-blue-600 text-white rounded">
                                + Add Row
                            </button>
                        </div>
                        <div v-if="form.vital_signs_log.length > 0" class="overflow-x-auto">
                            <table class="w-full text-sm text-left border border-gray-300 rounded">
                                <thead class="bg-gray-100 dark:bg-gray-700">
                                    <tr>
                                        <th class="px-2 py-1 font-semibold text-gray-600 dark:text-gray-200">Time</th>
                                        <th class="px-2 py-1 font-semibold text-gray-600 dark:text-gray-200">Temp</th>
                                        <th class="px-2 py-1 font-semibold text-gray-600 dark:text-gray-200">Pulse</th>
                                        <th class="px-2 py-1 font-semibold text-gray-600 dark:text-gray-200">Respiration</th>
                                        <th class="px-2 py-1 font-semibold text-gray-600 dark:text-gray-200">BP</th>
                                        <th class="px-2 py-1 font-semibold text-gray-600 dark:text-gray-200">GCS</th>
                                        <th class="px-2 py-1"></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr v-for="(row, i) in form.vital_signs_log" :key="i" class="border-t border-gray-200">
                                        <td class="px-1 py-1"><input type="time"   v-model="row.time"        class="w-full px-1 py-0.5 border border-gray-300 rounded text-sm dark:bg-gray-700 dark:text-white" /></td>
                                        <td class="px-1 py-1"><input type="number" v-model="row.temp"        class="w-full px-1 py-0.5 border border-gray-300 rounded text-sm dark:bg-gray-700 dark:text-white" step="0.1" placeholder="°C" /></td>
                                        <td class="px-1 py-1"><input type="number" v-model="row.pulse"       class="w-full px-1 py-0.5 border border-gray-300 rounded text-sm dark:bg-gray-700 dark:text-white" placeholder="bpm" /></td>
                                        <td class="px-1 py-1"><input type="number" v-model="row.respiration" class="w-full px-1 py-0.5 border border-gray-300 rounded text-sm dark:bg-gray-700 dark:text-white" placeholder="br/min" /></td>
                                        <td class="px-1 py-1"><input type="text"   v-model="row.bp"          class="w-full px-1 py-0.5 border border-gray-300 rounded text-sm dark:bg-gray-700 dark:text-white" placeholder="120/80" /></td>
                                        <td class="px-1 py-1"><input type="number" v-model="row.gcs"         class="w-full px-1 py-0.5 border border-gray-300 rounded text-sm dark:bg-gray-700 dark:text-white" min="3" max="15" /></td>
                        ...
                        <p v-else class="text-xs text-gray-400 mt-1">No rows yet. Click "+ Add Row" to record serial vitals.</p>
                    </div>
                </div>
            </section>
            ...
            <section>
                ...
                <div class="p-3 border border-dashed border-gray-400 rounded-md">
                    <div class="grid grid-cols-2 gap-3 lg:grid-cols-4 items-end">
                        <div>
                            <label class="block m-1 text-base text-gray-600 dark:text-gray-200">Eye Opening <span class="text-xs text-gray-400">(1-4)</span></label>
                            <select v-model="form.gcs_eye" class="w-full px-3 py-2 border border-gray-300 rounded-md text-base dark:bg-gray-700 dark:text-white">
                        ...
                        <div>
                            <label class="block m-1 text-base text-gray-600 dark:text-gray-200">Best Verbal <span class="text-xs text-gray-400">(1-5)</span></label>
                            <select v-model="form.gcs_verbal" class="w-full px-3 py-2 border border-gray-300 rounded-md text-base dark:bg-gray-700 dark:text-white">
                                <option :value="null">—</option>
                        ...
                        <div>
                            <label class="block m-1 text-base text-gray-600 dark:text-gray-200">Best Motor <span class="text-xs text-gray-400">(1-6)</span></label>
                            <select v-model="form.gcs_motor" class="w-full px-3 py-2 border border-gray-300 rounded-md text-base dark:bg-gray-700 dark:text-white">
                                <option :value="null">—</option>
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 mt-1 text-center">15 = Best · ≤8 = Comatose · 3 = Unresponsive</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section>
                ...
                <div class="grid grid-cols-1 gap-3 p-3 border border-dashed border-gray-400 rounded-md lg:grid-cols-2">
                    <div>
                        <label class="block m-1 text-base text-gray-600 dark:text-gray-200">Diagnosis / Working Impression</label>
            ...
            <section>
                ...
                <div class="grid grid-cols-1 gap-3 p-3 border border-dashed border-gray-400 rounded-md lg:grid-cols-2">
                    ...
                    <div><CustomInput name="Hospital Name"       v-model="form.hospital_name" /></div>
                    <div><CustomInput name="Hospital Department" v-model="form.hospital_department" /></div>
                    <div class="lg:col-span-2">
                    ...
                    <div><CustomInput name="Advanced Call By"   v-model="form.advanced_call_by" /></div>
                    <div><CustomInput name="Call Received By"   v-model="form.call_received_by" /></div>
                </div>
            </section>
            ...
            <section>
                ...
                <div class="grid grid-cols-1 gap-3 p-3 border border-dashed border-gray-400 rounded-md lg:grid-cols-2">
                    ...
                    <div class="lg:col-span-2">
                        <label class="block m-1 text-base font-medium text-gray-600 dark:text-gray-200">Types of Vehicle Involved</label>
                        <div class="flex flex-wrap gap-4">
                            <label v-for="opt in vehicleTypeOptions" :key="opt.value"
                                class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300 cursor-pointer">
                                <input type="checkbox" :value="opt.value" v-model="form.vehicle_types_involved" class="rounded" />
                                {{ opt.label }}
                            </label>
                        </div>
                    </div>
            ...
            <section>
                ...
                <div class="grid grid-cols-1 gap-3 p-3 border border-dashed border-gray-400 rounded-md lg:grid-cols-3">
                    <div><CustomInput name="Accomplished &amp; Endorsed by" v-model="form.accomplished_endorsed_by" /></div>
                    <div><CustomInput name="Noted by"                       v-model="form.noted_by" /></div>
                    <div><CustomInput name="Endorsement Received by"        v-model="form.endorsement_received_by" /></div>
                </div>
            </section>
