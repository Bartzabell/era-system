...
import {
    PhX, PhPlus, PhFloppyDisk, PhMapPin, PhXCircle,
    PhCheckCircle, PhMagnifyingGlass,
...
const props = defineProps<{
    ...
    incidents: Array<{
        id: number; incident_name: string; emergency_id: number
        base_severity: number; base_time: number; base_resources: number; base_secondary: number
    }>
...
const recomputeDistanceFromSite = async () => {
    ...
    try {
        const res  = await fetch(`https://router.project-osrm.org/route/v1/driving/${sLng},${sLat};${iLng},${iLat}?overview=false`)
        const data = await res.json()
        form.distance_km = data.code === 'Ok' && data.routes?.length
            ? parseFloat((data.routes[0].distance / 1000).toFixed(2))
            : haversineKm(iLat, iLng, sLat, sLng)
    } catch {
...
const priorityDisplay = computed(() =>
    form.priority_score
        ? { score: form.priority_score, level: form.priority_level, label: form.priority_label }
        : null
)
...
const reverseGeocodeAddress = async (lat: number, lng: number): Promise<string> => {
    try {
        ...
        return nearby
            ? `${nearby}${(p.city || p.town || p.municipality) ? `, ${p.city || p.town || p.municipality}` : ''}`
...
const initMiniMap = async () => {
    ...
    if (siteId) {
        ...
        if (siteCoords) {
            ...
            L.marker(siteCoords, { icon: siteIcon }).addTo(miniMap).bindTooltip(site?.site_name ?? 'Site', { permanent: false })

            try {
                const [sLat, sLng] = siteCoords, [iLat, iLng] = coords
                const res  = await fetch(`https://router.project-osrm.org/route/v1/driving/${sLng},${sLat};${iLng},${iLat}?overview=full&geometries=geojson`)
                const data = await res.json()
                if (data.code === 'Ok' && data.routes?.length) {
                    const route = data.routes[0]
                    miniMapRoadDistance.value = parseFloat((route.distance / 1000).toFixed(2))
                    L.geoJSON(route.geometry, { style: { color: '#f97316', weight: 4, opacity: 0.85 } }).addTo(miniMap)
                    const lats = route.geometry.coordinates.map((c: number[]) => c[1])
                    const lngs = route.geometry.coordinates.map((c: number[]) => c[0])
                    miniMap.fitBounds([[Math.min(...lats), Math.min(...lngs)], [Math.max(...lats), Math.max(...lngs)]], { padding: [24, 24] })
                } else {
                    L.polyline([siteCoords, coords], { color: '#f97316', weight: 3, dashArray: '6 6', opacity: 0.75 }).addTo(miniMap)
...
const searchAddress = async () => {
    ...
    try {
        searchResults.value = await (await fetch(
...
const initMap = () => {
    nextTick(() => {
        ...
        if (form.map_coordinates) {
            const p = parseCoordsFromString(form.map_coordinates)
            if (p) { center = p; markerPos.value = center }
        }
...
const closeMapModal = () => {
    showMap.value = false; searchQuery.value = ''; searchResults.value = []
    resolvedAddress.value = ''; map?.remove(); map = null; mapMarker = null
}
    ...
    navigator.geolocation
        ? navigator.geolocation.getCurrentPosition(
            ({ coords }) => {
                ...
                map?.setView(markerPos.value, 16); mapMarker?.setLatLng(markerPos.value)
                reverseGeocode(coords.latitude, coords.longitude); isLocating.value = false
            },
            async () => locateByIp(),
            { timeout: 8000 }
          )
...
const locateByIp = async () => {
    try {
        ...
        if (data.latitude && data.longitude) {
            ...
            map?.setView(markerPos.value, 13); mapMarker?.setLatLng(markerPos.value)
            reverseGeocode(markerPos.value[0], markerPos.value[1])
...
const submitForm = () => {
    ...
    isEdit.value
        ? form.post(`/dispatch/${props.record.id}`, options)
        : form.post('/dispatch', options)
}
