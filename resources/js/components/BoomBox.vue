    ...
    <Popover v-model:open="open">
        <PopoverTrigger as-child>
            <Button variant="outline" role="combobox" :aria-expanded="open"
                ...
                <div class="flex items-center text-[10px] 2xl:text-base gap-2 overflow-hidden">
                    <template v-if="!multiple">
                        <span :class="[
                            'truncate text-[10px] 2xl:text-base',
                        ...
                        <button v-if="!readonly && !disabled" type="button"
                            class="ml-1 hover:bg-gray-200 text-[10px] 2xl:text-base rounded-full p-0.5"
                    ...
                    <template v-if="multiple && selectedItems.length > 0">
                        <div v-for="item in selectedItems.slice(0, 5)" :key="item.id"
                            class="flex items-center px-1 text-[10px] 2xl:text-xs bg-gray-100 rounded-sm">
                        ...
                        <span v-if="selectedItems.length > 2" class="text-[10px] 2xl:text-xs text-gray-500">
        ...
        <PopoverContent v-if="!readonly && !disabled" class="w-full text-black p-0 border border-gray-400">
            <Command>
                <CommandInput v-model="searchValue" class="text-right text-xs" :placeholder="`Search`" />
                <CommandList>
                    ...
                    <CommandGroup>
                        <CommandItem class="font-medium" v-for="item in filteredItems" :key="item.id" :value="item"
                            ...
                            <div class="flex text-[10px] 2xl:text-sm flex-col">
                                {{ item[labelField] }}<br><span class="text-[10px] 2xl:text-xs opacity-70">
