...
<template>
    <div>
        <label class="text-[14px] 2xl:text-base font-medium text-gray-600 dark:text-gray-200">{{ name }}</label>
        <div :class="{ '!border-red-500 !border-2': message }"
            ...
            <input ref="inputRef" v-model="displayValue" :placeholder="placeholder" :readonly="readonly"
                :disabled="disabled" @input="handleInput" v-bind="inputAttributes" :class="[
                    'w-full py-[5px] rounded-xl focus-within:ring-1 dark:bg-gray-500 dark:text-gray-200 text-[14px] 2xl:text-lg dark:focus-within:ring-sky-300 focus-within:ring-sky-900 ring-sky-100 focus:outline-none pl-2',
